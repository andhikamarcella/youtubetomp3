...
const statusFromFile = async (meta = {}) => {
  try {
    ...
    return {
      ...
      uploadedAt: meta.uploadedAt || meta.updatedAt || stat.mtime.toISOString(),
    ...
    const error = new Error(err?.message || "cookies_status_failed");
    error.code = err?.message?.startsWith("cookies_") ? err.message : (err?.code || "cookies_status_failed");
    throw error;
  }
};
...
export const getCookiesStatus = async () => {
  ...
  const meta = await readMeta();
  try {
    const record = await statusFromFile(meta);
    if (record) {
      if (!meta.exists || meta.sizeBytes !== record.sizeBytes || meta.hash !== record.hash || meta.health === "missing") {
        await writeMeta({
          exists: true,
    ...
    if (meta.exists || meta.health !== "missing") {
      await writeMeta({ exists: false, sizeBytes: 0, bytes: 0, health: "missing", updatedAt: null });
    }
    return {
      exists: false,
      sizeBytes: 0,
