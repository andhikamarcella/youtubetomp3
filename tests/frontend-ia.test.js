...
test('mobile clean header stays above drawer layers and can toggle drawer', () => {
  ...
  assert.match(html, /background: var\(--surface\) !important;/);
  assert.match(html, /box-shadow: 0 12px 34px rgba\(0, 0, 0, \.28\) !important;/);
  assert.match(html, /\.ytclean-mobilebar \{ display: flex !important; position: fixed; top: calc\(env\(safe-area-inset-top\) \+ 10px\); left: 12px; right: 12px; \}/);
