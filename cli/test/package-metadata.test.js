...
const releaseBranch = 'release/ytconv-1.6.7-identity-ui';

function repoFile(relative) {
  return path.join(repositoryDirectory, relative);
}

test('1.6.7 exposes complete pinned identity and security metadata', () => {
  ...
  assert.equal(manifest.installer.npm, 'https://registry.npmjs.org/ytconv/-/ytconv-1.6.7.tgz');
  assert.equal(manifest.installer.release, 'https://github.com/andhikamarcella/youtubetomp3/releases/tag/ytconv-v1.6.7');
...
test('Android package is current and keeps subtitles off by default', () => {
  ...
  assert.match(gradle, /versionCode 10607/u);
  assert.match(gradle, /versionName '1\.6\.7'/u);
...
test('README presents the 1.6.7 identity and browser-login behavior', () => {
  assert.match(readme, /1\.6\.7/u);
