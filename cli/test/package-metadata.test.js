...
test('1.5.7 exposes complete package-manager metadata', () => {
  ...
  assert.match(manifest.releaseNotes, /FFmpeg engine automatically/u);
  assert.equal(manifest.installer.type, 'Tarball');
  assert.equal(manifest.installer.url, 'https://registry.npmjs.org/ytconv/-/ytconv-1.5.7.tgz');
...
test('npm test is restricted to the YTConv CLI test directory', () => {
  ...
  assert.equal(
    ...
    false,
    'the 1.5.7 release branch must not contain the web account-server workflow',
  );
});
