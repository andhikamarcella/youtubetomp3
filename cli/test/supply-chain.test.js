...
test('1.7.5 publishes recognized license types and no lifecycle install hooks', () => {
...
test('identity dependencies are exact and represented by the committed lock graph', () => {
  ...
  assert.equal(lock.version, '1.7.5');
  assert.equal(lock.packages[''].version, '1.7.5');
});
...
test('README exposes versioned Socket TypeScript ISC and security documentation', () => {
  assert.match(readme, /badge\.socket\.dev\/npm\/package\/ytconv\/1\.7\.5/u);
