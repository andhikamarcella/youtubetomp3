...
def pip_install(*packages):
    commands = [
        [sys.executable, "-m", "pip", "install", "-U", "--no-cache-dir", "--break-system-packages"] + list(packages),
        [sys.executable, "-m", "pip", "install", "-U", "--no-cache-dir"] + list(packages),
    ]
...
def yt_dlp_args(options, output):
    args = common_args(options, output)
...
def diagnostics():
    ...
    rows = [
        ("YTConv iSH", VERSION), ("Python", sys.version.split()[0]), ("Shell", os.environ.get("SHELL", "-")),
