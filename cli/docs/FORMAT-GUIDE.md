...
YTConv 1.7.3 verifies that a real output file exists before reporting success. Repeating the same command is supported: the second run overwrites only when requested or returns the already valid output according to the selected archive/overwrite policy.
