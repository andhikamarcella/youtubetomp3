...
const oldVersion = '1.6.5';
const newVersion = '1.6.6';
...
async function replaceReleaseIdentity(relative) {
  try {
    const before = await read(relative);
    const after = before
      .replaceAll(oldVersion, newVersion)
      .replaceAll(oldBranch, releaseBranch);
...
async function filesUnder(relativeDirectory) {
  ...
  async function visit(directory) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) await visit(absoluteEntry);
      else if (entry.isFile()) output.push(path.relative(root, absoluteEntry));
    }
  }
...
await patch('cli/src/managed-browser.js', (source) => {
  let next = source.replace("import which from 'which';\n", '');
  next = replaceRequired(
    next,
    "import { privateChildEnvironment } from './terminal-style.js';\n",
    "import { privateChildEnvironment } from './terminal-style.js';\nimport { resolveCommandPath } from './command-path.js';\n",
    'managed-browser command-path import location',
  );
  next = replaceRequired(
    next,
    ...
    'managed-browser which resolver',
  );
  next = next
    .replace('const resolved = await executableExists(executable);', 'const resolved = await executableExists(executable, { environment: env, platform });')
    .replace('const resolved = await executableExists(candidate);', 'const resolved = await executableExists(candidate, { environment: env, platform });');
  return next;
});
...
manifest.releaseNotes = 'YTConv 1.6.6 preserves tested Windows EXE, DEB, RPM, Arch, Alpine APK, AppImage, Snap, Flatpak, Nix, Termux and Android packaging; keeps repaired iSH and Alpine update paths; and reduces npm supply-chain exposure with shell-free command discovery and embedded terminal logos.';
...
for (const directory of identityDirectories) {
  for (const relative of await filesUnder(directory)) await replaceReleaseIdentity(relative);
}
for (const relative of identityFiles) await replaceReleaseIdentity(relative);

await patch('cli/test/package-metadata.test.js', (source) => source
  .replace("assert.equal(manifest.releaseDate, '2026-08-04');", "assert.equal(manifest.releaseDate, '2026-08-05');")
  .replace("assert.deepEqual(Object.keys(manifest.dependencies).sort(), ['figlet', 'ink', 'react', 'which', 'ws']);", "assert.deepEqual(Object.keys(manifest.dependencies).sort(), ['ink', 'react', 'ws']);"));
